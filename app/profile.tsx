      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.statusUpdateCard}>
          <TextInput
            ...
            value={profile.statusUpdate || ""}
            onChangeText={(text) => {
              const updated = { ...profile, statusUpdate: text };
              setProfile(updated);
              AsyncStorage.setItem(PROFILE_STORAGE_KEY, JSON.stringify(updated)).catch(() => undefined);
            }}
            multiline
            maxLength={280}
          />
          {profile.statusGifUrl && (
            <View style={styles.statusGifContainer}>
              <Image source={{ uri: profile.statusGifUrl }} style={styles.statusGif} />
              <Pressable onPress={() => {
                const updated = { ...profile, statusGifUrl: undefined };
                setProfile(updated);
                AsyncStorage.setItem(PROFILE_STORAGE_KEY, JSON.stringify(updated)).catch(() => undefined);
          ...
          {profile.nowPlayingSong && (
            <View style={styles.nowPlayingBadge}>
              <MaterialIcons name="music-note" size={16} color="#8557D9" />
              <Text style={styles.nowPlayingText}>{profile.nowPlayingSong.title} • {profile.nowPlayingSong.artist}</Text>
              <Pressable onPress={() => {
                const updated = { ...profile, nowPlayingSong: undefined };
                setProfile(updated);
                AsyncStorage.setItem(PROFILE_STORAGE_KEY, JSON.stringify(updated)).catch(() => undefined);
              }}>
                <MaterialIcons name="close" size={16} color="#8557D9" />
              </Pressable>
            </View>
          )}
          <View style={styles.statusActionButtons}>
            <Pressable style={({ pressed }) => [styles.statusActionButton, pressed && styles.pressed]}>
              <MaterialIcons name="image" size={20} color="#8557D9" />
              <Text style={styles.statusActionText}>GIF</Text>
            </Pressable>
            <Pressable style={({ pressed }) => [styles.statusActionButton, pressed && styles.pressed]}>
              <MaterialIcons name="music-note" size={20} color="#8557D9" />
              <Text style={styles.statusActionText}>Song</Text>
            </Pressable>
          </View>
        </View>
