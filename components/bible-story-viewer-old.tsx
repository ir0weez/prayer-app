...
import { getUserNotesForVerse, loadProfiles, type UserNote, type CommentaryProfile } from '@/lib/commentary-system';
...
export function BibleStoryViewer({
  ...
  const [profiles, setProfiles] = useState<Record<string, CommentaryProfile>>({});
  ...
  const loadCommentary = async () => {
    ...
    const curatedData = await getAllCommentariesForVerse(book, chapter, verse.verse);
    const localNotes = await getUserNotesForVerse(book, chapter, verse.verse);
    const profilesData = await loadProfiles();
    const profilesMap = profilesData.reduce((acc, p) => ({ ...acc, [p.id]: p }), {});
    
    setCommentaries(curatedData);
    setUserNotes(localNotes);
    setProfiles(profilesMap);
    setIsCommentaryLiked(curatedData.length > 0 ? (curatedData[0]?.isLikedByUser ?? false) : false);
    setIsLoadingCommentary(false);
  };
