...
export async function loadUnifiedBible(): Promise<UnifiedBibleState> {
  try {
    ...
    if (data) {
      ...
      // Keep existing installs in sync when a newly supported book is added.
      // Older states may have a current status for a book but no chapter rows.
      let catalogChanged = false;
      if (!Array.isArray(state.chapters)) {
        state.chapters = [];
        catalogChanged = true;
      }
      if (!state.bookStatuses) {
        state.bookStatuses = {};
        catalogChanged = true;
      }
      for (const book of BIBLE_BOOKS) {
        ...
        const chapterCount = CHAPTER_COUNTS[book] || 1;
        for (let ch = 1; ch <= chapterCount; ch++) {
          if (!state.chapters.some((entry: BibleChapterStatus) => entry.book === book && entry.chapter === ch)) {
            state.chapters.push({ book, chapter: ch, isRead: false });
            catalogChanged = true;
          }
        }
      }
      if (catalogChanged) await saveUnifiedBible(state);
