...
describe('cleaned Isaiah commentary', () => {
  ...
  it('preserves chapter-one subsection order and exact verse-header ranges', () => {
    ...
    expect(rangeNote?.startVerse).toBe(8);
  ...
  it('integrates Isaiah into structured sections and range-aware lookup', () => {
    ...
    expect(notes[0]?.startVerse).toBe(8);
    expect(notes[0]?.endVerse).toBe(9);
  });

  it('creates one carousel group per exact verse header inside a subsection', () => {
    const fakePractices = CLEANED_ISAIAH_CHAPTERS[1]?.find((section) => section.title === 'Fake Practices');
    const groups = groupCommentariesByRange(fakePractices?.entries ?? []);
